...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(DeformableBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class DeformableBottleneck(nn.Module):
    """Deformable bottleneck using DeformableConv2d."""

    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super(DeformableBottleneck, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        # Using deformable convolutions for both layers in the bottleneck
        self.cv1 = DeformableConv2d(c1, c_, kernel_size=k[0], stride=1)
        self.cv2 = DeformableConv2d(c_, c2, kernel_size=k[1], stride=1)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        """Forward pass using deformable convolutions."""
